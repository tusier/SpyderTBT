...
class WtoOtherSpider(scrapy.Spider):
    name = "wto_other"
    allowed_domains = ["www.tbtsps.cn"]
    ...
    def start_requests(self):
        ...
        for i in range(1, 2):  # 第一次全爬500，之后改成3
            data = {"title": "", "startPublishDate": "", "endPublishDate": "", "page": i, "type": "7",
                    "rows": 20}  # 请求参数
            yield scrapy.Request(url=url, method='POST', body=json.dumps(data), headers=self.headers,
                                 callback=self.parse)
